...
/**
 * Get current timestamp in format 'YYYY-MM-DD'
 */
export const getTimestamp = () => format(Date.now(), 'YYYY-MM-DD')

/**
 * Compile the array in a string with units and values
 * @param {Array} data
 * @param {String} unit
 * @returns
 */
export const compileDataString = (data, unit, spliter) => {
  if (data != null && Array.isArray(data)) {
    return data.map((d) => `${formatThousands(d.val, {})}${unit}`).join(spliter)
  } else {
    return `${formatThousands(data, {})}${unit}`
  }
}
