...
@click.command(short_help="Population raster styling raster")
@click.option(
    "-i",
    "--raster_input",
        ...
        with MemoryFile() as memfile:
            with memfile.open(**profile) as mem:
                ...
                for index, w in src.block_windows(1):
                    ...
                    out_arr = np.where((arr > 0) & (arr <= 100), 1, out_arr)
                    out_arr = np.where((arr > 100) & (arr <= 250), 2, out_arr)
                    out_arr = np.where((arr > 250) & (arr <= 400), 3, out_arr)
                    out_arr = np.where((arr > 400) & (arr <= 550), 4, out_arr)
                    out_arr = np.where(arr > 550, 5, out_arr)
